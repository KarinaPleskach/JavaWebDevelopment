...
public class ByStyleTest {

    private static Specification specification;

    @Before
    public  void before() throws DAOException {
        specification = new SpecificationFabric("POP").getSpecification("style");
    }

    @Test
    public void matchTrueTest() {
        Record record = new Song("a", new Duration(2, 2), new Band("Animal"), Styles.POP, Timbre.TENOR);
    ...
    public void matchFalseTest() {
        Record record = new Song("a", new Duration(2, 2), new Band("Animal"), Styles.ROCK, Timbre.BARITONE);
        boolean actual = specification.match(record);
        assertFalse(actual);
    }
